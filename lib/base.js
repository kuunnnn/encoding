 ...
 * @return {string}
 */
function IEEE754Float64_encode(num) {
  // Sign ： 1 bit（第63个bit）
  let sign = num > 0 ? "0" : "1";
  ...
  if (exponent.length >= 11) {
    ...
    exponent =
      new Array(11 - Math.min(exponent.length, 11)).fill("0").join("") +
      exponent;
  }
  ...
  if (fraction.length >= 52) {
    ...
    fraction += new Array(52 - Math.min(fraction.length, 52))
      .fill("0")
      .join("");
  }
 ...
 * @return {Number}
 */
function IEEE754Float64_decode(numb) {
 ...
 * @todo 和 64 合并到一个函数
 */
function IEEE754Float32_encode(num) {
  // Sign ： 1 bit（第31个bit）
  let sign = num > 0 ? "0" : "1";
  ...
  if (exponent.length >= 8) {
    ...
    exponent =
      new Array(8 - Math.min(exponent.length, 8)).fill("0").join("") + exponent;
  }
  // Fraction ：52 bits （第 51 至 0 共 52 个bits）
  let fraction = "";
  if (dotIndex > index) {
  ...
  if (fraction.length >= 23) {
    ...
    fraction += new Array(23 - Math.min(fraction.length, 23))
      .fill("0")
      .join("");
  }
  return sign + exponent + fraction;
 ...
 * @todo 和 64 合并到一个函数
 */
function IEEE754Float32_decode(numb) {
  if (numb[0] === "0" && numb[1] === "b") {
    numb = numb.slice(2);
  }
  // Sign ： 1 bit（第31个bit）
...
module.exports = {
  ...
  IEEE754Float64_encode,
  IEEE754Float32_encode,
  IEEE754Float64_decode,
